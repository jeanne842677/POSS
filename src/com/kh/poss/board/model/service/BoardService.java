...
public class BoardService {
   ...
   public List<Board> selectBoardList(String userId, Criteria cri) {
      List<Board> boardList = null;
      Connection conn = template.getConnection();
      try {
         boardList = boardDao.selectBoardList(userId, cri, conn);
      } finally {
         template.close(conn);
      }
      return boardList;
   ...
   public List<Board> searchBoardList(String userId, String searchKeyword, Criteria cri) {
      List<Board> searchList = null;
      Connection conn = template.getConnection();
      try {
         searchList = boardDao.searchBoardList(userId,searchKeyword,cri,conn);
      } finally {
         template.close(conn);
      }
      return searchList;
   ...
   public int getSearchCount(String userId, String searchKeyword) {
	   Connection conn = template.getConnection();
	   int count = 0;
	   
	   try {
		   count = boardDao.getSearchCount(conn, userId, searchKeyword);
		   template.commit(conn);
	   } catch (Exception e) {
		   template.rollback(conn);
		   throw e;
