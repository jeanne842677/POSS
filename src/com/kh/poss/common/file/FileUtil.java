...
public class FileUtil {
	...
	//파일 정보가 담겨있는 FileDTO와 MultipartRequest를 담고 있는 Map을 반환
		...
		try {
			...
			while((part = parser.readNextPart()) != null) {
				if(part.isFile()) {
					...
					//MultipartParser는 form 태그에 input[type=file]이 존재하면, 사용자가 파일을 첨부하지 
					//않더라도 FilePart객체를 생성해서 MultipartParser객체에 넘겨준다.
			...
			paramMap.put("com.kh.poss.files", fileDtoList);
		} catch (Exception e) {
