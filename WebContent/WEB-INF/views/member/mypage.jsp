...
<body>
		...
		<nav class="navbar">
			<div class="nav_wrap">
				...
				<div class="nav_group">
					...
					<div class="nim">님!</div>
					<button type="button" class="btn btn-secondary" id="main_button"
	...
	<div class="wrap">
		...
		<div class="modify_my_info">
			<button class="btn btn-lg btn-primary" type="button" id="modify_btn"
				onclick="modifyInfo()">내정보 수정</button>
		</div>
	...
	<div class="info_page_link">
		...
		<button class="btn btn-lg btn-primary" type="button"
			id="reservation_link_btn" onclick="location.href='/member/info-page'">>
			바로가기</button>
	</div>
	</div>
	</div>
	...
	<div class="delete">
		<div>
			<button type="button" class="btn btn-secondary"
				id="delete_account_btn" onclick="deleteUser()">> 회원탈퇴</button>
		</div>
	</div>
...
function deleteUser(){
	   modal3();
	   setModalTitle('modal3','비밀번호를 입력해주세요');
	   
	   
	   
	   setConfirmFunc = function confirmPw(){
		   
		   let password = document.getElementById('confirmPw').value;
           if(password == '${authentication.password}'){
           	modal2();
                 setModalTitle('modal2','회원 탈퇴');
                 setModalBody('modal2', '탈퇴 되었습니다.');
                 
                 setOkayFunc = ()=>{
                			
                				fetch("/member/deleteUser?password="+ password, {method: "POST"})
                				.then(res => res.text())
                				.then(text => {
                					if (text == "available") {
                						alert("탈퇴 되었습니다.");
			  ...
			  modal2();
              setModalTitle('modal2','회원 탈퇴 실패');
               setModalBody('modal2', '아이디 및 패스워드가 올바르지 않습니다.');
